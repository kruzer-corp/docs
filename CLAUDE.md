...
**PIM** — the spec is generated by a script in the sibling `pim-api` repo (`scripts/generate-openapi.ts`), which parses controllers in `src/features/*/actions/*` and emits the spec:
...
**OMS** — the spec is exported from the sibling `oms` repo rather than generated by a script here; the export procedure lives in that repo. Save the result over `oms/api/openapi.json`, then commit and merge.

The raw export is **not publishable as-is** — it needs a review pass before commit. Do not paste a fresh export straight in: diff it against the committed file first and carry over the adjustments already applied, since the file in this public repo must contain only what is meant to be public.
